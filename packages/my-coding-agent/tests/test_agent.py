...
def test_build_coding_tools_returns_default_set(tmp_path):
    tools = build_coding_tools(tmp_path)
    names = sorted(t.name for t in tools)
    assert names == ["bash", "edit", "list_files", "read", "search", "write"]
...
def test_coding_agent_auto_registers_file_tools(tmp_path):
    ...
    assert {"list_files", "search", "read", "write", "edit", "bash"} <= names
