...
class MemoryStore:
    ...
    def list_entries(self, target: TargetType) -> list[str]:
        """Return a defensive copy of the current in-memory entries for UI/status views."""
        if target not in self.files:
            raise ValueError(f"Invalid target {target!r}")
        return list(self._entries[target])

    def usage(self, target: TargetType) -> tuple[int, int]:
        """Return ``(used_chars, limit_chars)`` for a memory target."""
        if target not in self.files:
            raise ValueError(f"Invalid target {target!r}")
        text = ENTRY_DELIMITER.join(self._entries[target])
        return len(text), self.limits[target]
