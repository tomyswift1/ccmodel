...
本模块只做上下文“视图变换”（非破坏，绝不修改传入 list）；完整历史由 Session 管。
"""
...
CHARS_PER_TOKEN = 4
DEFAULT_CONTEXT_BUDGET = 100_000  # Default logical budget; configurable by the product layer.
...
def snip_messages(messages: list[Message], max_messages: int = 50) -> list[Message]:
    """L2：len > max_messages → 留头 3 + 尾 (max-4)，中间删，插一条 [snipped N] 占位。
...
def micro_compact(
    ...
    """L3：非最近 keep_recent 条、content > min_chars 的 tool 消息 → content 换占位符。
...
def budget_tool_results(
    ...
    """L1：content 超 max_chars 的 tool 消息 → 落盘到 results_dir/<tool_call_id>.txt，视图换预览。
...
class ContextManager:
    ...
    def _prepare_with_cache(self, messages: list[Message]) -> list[Message]:
        ...
        压缩后继续对话产生的新消息同样可能超限——对新增段跑 L1/L3（先将大结果落盘，再压缩较旧工具结果），
        整个视图再跑 L2（对话继续增长后快照也需进一步压缩）。
        """
...
class ContextSessionBridge:
    """ContextManager ↔ Session 持久化桥：缓存 entry 的读写、L1 落盘目录。
    ...
    def results_dir(self) -> Path | None:
        """L1 落盘目录：session 所在 workspace 的 .my_agent_core/tool-results/。"""
