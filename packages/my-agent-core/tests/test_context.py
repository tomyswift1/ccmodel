...
def test_snip_keeps_pairing():
    """L2：>50 消息裁中间 + [snipped] 占位，不拆 assistant(tool_calls)+tool 配对（#5）。"""
...
def test_micro_compact_old_tool_results():
    """L3：旧 tool 消息（>200 字符、非最近 5 条）→ 占位，metadata 保留（#6）。"""
...
def test_budget_tool_results_persists_large(tmp_path):
    """L1：超大 tool 消息 → 落盘 + 视图换预览；原 messages 未修改（#4）。"""
...
async def test_cache_branch_applies_free_layers_to_newly(tmp_path):
    """缓存分支对新增段也跑免费层（L1 落盘 + L3 占位）——压缩后免费层不失效。"""
    llm = FakeLLM([_response(content="## Goal\n...")])
    ...
    # 压缩后新增：6 条旧 tool（L3 占位）+ 1 条超大 tool（L1 落盘 → 视图变 preview → 不超阈）
