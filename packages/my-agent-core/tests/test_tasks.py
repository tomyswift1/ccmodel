...
@pytest.mark.anyio
async def test_task_tool_reports_subagent_failure_as_structured_error(tmp_path: Path):
    """The task bridge raises on child failure so Tool.execute returns ok=False."""
    _write_agent(tmp_path, "worker", description="worker", content="Worker prompt")
    manager = SubagentManager([tmp_path])
    parent = Agent(
        llm=RaisingLLM(),
        tools=[],
        session=Session(path=tmp_path / "parent-error.jsonl"),
    )
    task_tool = make_task_tool(manager, parent)
    result = await task_tool.execute({"prompt": "fail", "agent_type": "worker"})
    assert result.ok is False
    assert "Subagent 'worker' failed" in result.error
