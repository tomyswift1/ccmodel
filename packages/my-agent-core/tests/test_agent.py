...
@pytest.mark.anyio
async def test_empty_model_response_has_explicit_stop_reason():
    """Empty content with no tool calls must not look like a successful silent completion."""
    llm = FakeLLM([_response(content="")])
    events = []
    agent = _agent(llm, hooks=[(AgentEnd, events.append)])
    answer = await agent.run("do something")
    assert answer == "(model returned an empty response; please retry)"
    assert events[-1].stop_reason == "empty_response"


@pytest.mark.anyio
async def test_model_exception_closes_agent_lifecycle():
    """A final provider failure emits AgentEnd(model_error) before surfacing to the CLI."""
    class BrokenLLM:
    ...
    events = []
    agent = _agent(BrokenLLM(), hooks=[(AgentEnd, events.append)])
