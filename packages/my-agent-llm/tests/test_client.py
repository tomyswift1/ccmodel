...
def test_config_or_kwargs_construction():
    """两种构造等价：传 Config 或散参。"""
    a = LLM(config=Config(api_key="k", model="m"))
    b = LLM(api_key="k", model="m")
    assert a.model == "m"
    assert b.model == "m"


def test_uses_openai_compatible_provider():
    """The focused project keeps one OpenAI-compatible provider boundary."""
    from my_agent_llm.providers.openai import OpenAIProvider

    llm = LLM(config=Config(api_key="k"))
...
def test_chat_falls_back_to_config_sampling_params():
    ...
    llm = LLM(config=Config(api_key="k", model="m", temperature=0.3, max_tokens=123))
...
def test_chat_omits_max_tokens_when_config_none():
    """config.max_tokens=None → chat 不注入 max_tokens 键。"""
    from my_agent_llm.models import Response

    llm = LLM(config=Config(api_key="k", model="m"))
